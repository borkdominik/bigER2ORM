from entity.certificate import Certificate
from entity.course import Course
from entity.course_with_exercise import CourseWithExercise
from entity.graduate_student_card import GraduateStudentCard
from entity.lecturer import Lecturer
from entity.person import Person
from entity.recognized_certificate import RecognizedCertificate
from entity.student import Student
from entity.student_card import StudentCard
from entity.student_study_program import StudentStudyProgram
from entity.study_program import StudyProgram

__all__ = [
    'Certificate',
    'Course',
    'CourseWithExercise',
    'GraduateStudentCard',
    'Lecturer',
    'Person',
    'RecognizedCertificate',
    'Student',
    'StudentCard',
    'StudentStudyProgram',
    'StudyProgram',
]
